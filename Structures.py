...
class matrix:
    ...
    def rotate(self) -> list[list]:
        """Creates a new Grid that is a version of the old one rotated clockwise once 90 degrees.
    ...
    def scanGridSeveralJR(self, target : any) -> set:
    ...
    def scanGridSeveralSR(self, target : any) -> set:
        """Scans the grid for multiple instances of your target. the scan goes ROW BY ROW, one after another.
...
class graphicalmatrix(matrix):
    """A class that represents the typical grid, bunch of rows and columns
        ...
        functionally graph paper.
        this ain't swiss cheese! all grids is either a square or a rectangle.
        this class allows for easier use of the turtle library

        Attributes:
            matrix (list[list]): the matrix itself
            rows (int): rows in grid
            columns (int): columns in grid
            refreshpending (bool): if an update is made but not shown in turtle, this bool will be true
    ...
    def getStats(self) -> list:
        """Gives you the rows and columns of the grid. you can access
            ...
            Returns 
            list: [rows, columns, refreshpending]
        """
        return (self.rows,self.columns,self.refereshpending)
    ...
    # TODO: this shit below requires alot of work
    ...
    def drawGrid(self, drawstart = 0, drawkey = {}, bgcolor = 'black'):
